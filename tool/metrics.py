...
def _truncate_response_set(response_set: List[str], max_token_length: int) -> List[str]:
    ...
    if token_count < max_token_length:
        print(
            "\033[93mWarning: The response did not reach the maximum token limit.\033[0m"
        )
...
class DistinctNgrams:
    ...
    def __init__(
        self, n: int = 2, tokenizer_name: str = "split", use_lower_case: bool = True
    ) -> None:
        self.n = n
    ...
    def _tokenize(self, line: str) -> List[str]:
        ...
        if self.use_lower_case:
            line = (
                line.lower()
            )  # this is because M2M is lowercased dataset, thus for fair comparison we need to lowercase the other datasets as well.
        if self.tokenizer_name == "split":
            return [
                e for e in line.replace(".", "").replace("\n", "").split(" ") if e != ""
            ]  # The same as Tevet's implementation
        elif self.tokenizer_name == "nltk":
            return word_tokenize(line)
        else:
            raise ValueError(
                "Invalid tokenizer_name. Valid options are 'split' and 'nltk'."
            )
    ...
    def _calculate_unique_ngram_ratio(
        self, ngram_lists: List[List[Tuple[str, ...]]]
    ) -> float:
    ...
    def calculate_normalized_distinct_n(
        self,
        response_set: List[str],
        return_avg: bool = True,
        max_token_length: int = 7012,
        iterations: int = 100,
        seed: int = 42,
        ...
        for _ in range(iterations):
            ...
            truncated_response_set = _truncate_response_set(
                shuffled_response_set, max_token_length
            )
